...
class Tracker:
    ...
    def draw_ellipse(self, frame, bbox, color, track_id=None):
        ...
        if track_id is not None:
            cv2.rectangle(frame, (x_center - 20, y2 - 10), (x_center + 20, y2 + 10), color, cv2.FILLED)
            cv2.putText(frame, str(track_id), (x_center - 10, y2 + 5), cv2.FONT_HERSHEY_SIMPLEX, 
                        0.6, (0, 0, 0), 2)
    ...
    def draw_team_ball_control(self, frame, frame_num, team_ball_control):
        ...
        team_1_control = np.mean(team_ball_control_till_frame == 1)
        team_2_control = np.mean(team_ball_control_till_frame == 2)

        cv2.putText(frame, f"Team 1 Ball Control: {team_1_control * 100:.2f}%", (1400, 900), 
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
        cv2.putText(frame, f"Team 2 Ball Control: {team_2_control * 100:.2f}%", (1400, 950), 
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
